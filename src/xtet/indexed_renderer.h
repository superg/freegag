...
bool blit_transparent(const IndexedBitmap &source, IndexedFramebuffer destination, int32_t x, int32_t y, bool flip_x = false, bool flip_y = false);
bool blit_rli_frame(const RliFrameRecord &frame, IndexedFramebuffer destination, int32_t x, int32_t y);
bool blit_rli_frame_canvas(const RliFrameRecord &frame, uint32_t canvas_width, uint32_t canvas_height, IndexedFramebuffer destination, int32_t x, int32_t y, bool flip_x = false, bool flip_y = false);
...
bool collect_figurine_board_regions(const FallingFigurine &figurine, uint32_t framebuffer_width, uint32_t framebuffer_height, std::vector<FigurineRenderRegion> &regions, int32_t origin_x = 0,
    int32_t origin_y = 0);
bool fill_figurine_board_regions(const FallingFigurine &figurine, IndexedFramebuffer framebuffer, uint8_t fill_index, int32_t origin_x = 0, int32_t origin_y = 0);
...
bool render_match_blink_sequence(const FallingFigurine &first, const FallingFigurine &second, const ActionDefinition &action, const std::vector<RliAnimation> &animations,
    IndexedFramebuffer framebuffer, const std::function<void(const FigurineRenderRegion &)> &dirty_callback, const std::function<void(uint32_t)> &delay_callback, int32_t origin_x = 0,
    int32_t origin_y = 0);
