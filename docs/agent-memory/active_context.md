...
- Changed the default-ON modern Window-mode startup placement as requested. `InitializeGagApplication` still uses `AdjustWindowRect` to preserve the exact 640x480 client and fixed 648x534 test outer size, but passes `CW_USEDEFAULT` for both initial x/y coordinates instead of centering from desktop metrics. Later explicit game layout transitions are unchanged, and fixes-OFF retains original placement. Both startup variants and CTest 3/3 pass. Ghidra is unchanged because this is explicitly non-original compatibility behavior.
