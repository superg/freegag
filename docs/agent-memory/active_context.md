...
- Restored the small all-build synthesized-resource implementation after
  ...
  `[300,80)-(620,390)` and `[12,402)-(135,430)` with pixels extracted from the
  accepted `HELP_BACKGROUND_INDEXED_8BIT.BMP`. Their 102,644 palette indices are
  stored as one 65,288-byte zlib stream in `save_load_bg_patch.h` and applied
  after both source blits. `tools/generate_save_load_bg_patch.py` now reproduces
  the embedded patch from the accepted indexed BMP and validates its format.
  The x64 Debug production executable and focused test targets build, all four
  configured CTest tests pass, and formatting checks pass.

- Implemented the scripted Load screen milestone. `SAVELOAD.CFG::LOAD` is founded directly on the recovered Help scene: the original `TAG_NEXT`, `TAG_BACK`, `TAG_EXIT`, and `MENU_3S` hierarchy supplies both Help navigation controls plus Exit, with only the final Previous/Next events remapped to cycle saves. The recovered disabled menu images `FSCR0011.BMP` through `FSCR0014.BMP` and `FSCR0016.BMP` through `FSCR0017.BMP` are static, noninteractive script images at their original `DIALOG.CFG` positions. The original Load position instead reuses the recovered `MENU_3S` behavior: its `(7,171,266,25)` zone shows `FSCR0005.BMP` on hover and sends the same Load message as the preview; `FSCR0015.BMP` is not referenced. Because this screen combines main-menu and Help controls under one `MM::SEL` while their recovered templates use different background-clear rectangles, fast pointer motion could jump directly between priority zones without entering `z_MAIN` and leave the prior highlight visible. Save/load's private template now clears both the left-menu and Help-control rectangles on every hover/reset, and entering the thumbnail transparently performs the same reset before its click event. Ordinary blank script layers add a native `320x240` `COMMENT.BMP` preview vertically centered at `(294,120)` and an ellipsized `FONT2.RUS` caption six pixels beneath it at `(302,366)`. The caption draws from local `(0,0)`, matching stock `/INPSTR` rather than centering, and both static and editing states use palette foreground/frame indices `51:1`; original `HELP0000.BMP` verifies index 51 is its closest yellow at RGB `(233,185,79)`. The matching Load preview zone, Save name zone, and stock `/INPSTR` origin use those same coordinates. Controlled traces established two independent reasons those populated layers were invisible: ownerless scene links retained the compositor's inactive bit, and their 8-bit blank resources had no palette or rectangle callback on the high-color display path. Save/load explicitly activates both owned layers; the caption uses the active game palette, while the preview now installs the selected `COMMENT.BMP` palette as described above. The exact preview rectangle is also a Load zone and displays the normal CDF-backed pointer cursor on hover. Previous/Next cycle the mtime-sorted manual `*.GSF` collection, newest is selected initially, and either Load target enters the existing `START.CFG` transition without revalidation. All controller, collection, rendering, script, and private-message behavior remains in `save_load`; `startup` only forwards integration and supplies neutral engine services. Focused tests cover the shared coordinates, ownerless activation, palette-driven 8-to-16 callback selection, editor color arguments, and authoritative yellow palette entry. The x64 Debug build, all four configured CTest tests, formatting, and whitespace checks pass; runtime visual confirmation remains pending.
...
- Removed whole-text flicker from fixes-owned `/INPSTR` presentation without changing the recovered editor function, caret timing, input rules, or fixes-off path. The original redraw reacquires and clears its text scene before `BeginDisplaySceneUpdate`, allowing the compositor to expose the cleared buffer. A narrow production adapter now begins an outer update when the new text bounds fit the existing editor scene, adds the recovered acquire flag that retains the larger existing allocation, and releases the outer update only after the ordinary nested redraw commits. Growing text remains on the original unguarded reallocation path so allocation failure cannot remove a scene while an outer guard references it. Focused tests verify successful nested ordering plus guard cleanup after acquire and inner-begin failures. The x64 Debug production executable and test targets build and all four configured CTest tests pass; interactive Save editor confirmation remains pending.

- The attempted one-time post-initialization/atomic Save-preview change was runtime-disproved and fully reverted. The prior behavior is restored: the tree-rebuild callback activates and initially renders the synthesized layers, Save message 2105 redraws the retained thumbnail, and a flagged cached state with null `saved_memory` recaptures the bitmap while preserving its serialized state. A Debug-only, observation-only `save-preview-trace.log` then captured the actual intermittent failure. The retained snapshot was non-null and validated as a 77,878-byte BMP, both render attempts ran, and layer activation succeeded, but `SavePreview` resolved to an existing 288x288 scene rather than its declared 320x240 geometry, so both draws correctly stopped before palette setup or pixel copying. The scripted indices `524289`/`524290` were `0x80001`/`0x80002`, directly inside the engine's busy dynamically allocated scene pool starting at `0x80000`; recovered ownerless acquisition returns an existing same-index scene unchanged even when its dimensions do not fit. The private layers now use `0x70000`/`0x70001`, preserving their order below generic and input scenes while placing them in the verified gap above all stock fixed script indices and below the `0x80000` pool. No global scene-acquisition behavior changed. The script regression asserts the exact indices and their placement. The x64 Debug production executable and all test targets build, all four configured CTest tests pass, and formatting and whitespace checks pass. The diagnostic remains until repeated runtime Save/Load confirmation.
...
- Fixed the traced missing-autosave cause, awaiting runtime confirmation. `GR1` activation queried scene flags through `current_runtime_scene_identity`, the software cursor at original game-context `+0x95c`. It was null at root activation, so `ProcessStateActivation` returned status zero, falsely set `NOSAVE`, `/MESSAGE::3010` skipped serialization, and final shutdown saw `script_state == 0` and never invoked the writer. Original `GAG.EXE` uses game-context `+0x960`, which is the typed `current_runtime_resource`; the reconstruction now queries that field. The regression deliberately sets different cursor and gameplay-resource identities and asserts the resource identity reaches the status query. The temporary `autosave-trace.log` remains for one confirmation run. The production Debug build, all 3 configured tests, formatting, and whitespace checks pass.

- Fixed and runtime-confirmed software cursor leave/re-entry across the main menu, dossier, and gameplay. The diagnostic trace showed that leaving cleared the active pointer region and deferred the default scene under runtime flag `0x1000`, but re-entry never cleared the suspend flag; selecting the cursor region therefore deferred its scene, and the same-region fast path left the current cursor scene null forever. The fixes-only child procedure now balances that suspension on the first `WM_MOUSEMOVE` after `TME_LEAVE`, for both DLL-consumed and ordinary mouse paths. Runtime scene transition values and callbacks are pointer-sized so deferred identities survive AMD64 restoration. On leave, the child directly suspends the software cursor while retaining the hidden native cursor, then queues the outside pair needed to clear the active region; re-entry restores the appropriate software cursor. The user confirmed the cursor disappears outside and correctly reappears inside in all affected contexts. The earlier framebuffer redraw, custom-message forced resume, and deferred atomic-reset experiments remain removed. Focused regressions cover full-width re-entry restoration and the direct-suspend/outside-pair leave sequence. The temporary diagnostic tracer has been removed; the production Debug build, all 3 configured tests, formatting, and whitespace checks pass.

- Fixed the title-menu Save action appearing enabled at startup. `HandleCurrentStateActivation` was comparing its `RuntimeTreeNode` callback argument with the current mouse-region pointer, so every real startup activation was rejected and the root tree's `NOSAVE` flag never reached the menu snapshot. Original `GAG.EXE` confirms game-context offset `+0x964` is the current runtime-tree identity used by the script executor. The reconstruction now compares against `runtime_tree_identity`; the regression carries the resulting `0x800000` bit through `/MESSAGE::3010` and `/MESSAGE::1003` to `MM::SAVE_DIS`. Both startup-test variants, all 3 configured CTest tests, the production Debug build, and formatting pass. A controlled startup run against the original CDF data confirmed the live query receives `NOSAVE`.

- Fixed the high-color/x64 load-dialog Cancel freeze. `open_application_state_interactive` previously waited unconditionally for `runtime_resource_count` to reach zero after a canceled dialog, even though the high-color path had not torn down an 8-bit display and ordinary resource activity could keep the count nonzero indefinitely. The drain wait now runs only inside the 8-bit restore path. A regression test cancels with 16-bit display state and a nonzero resource count; both startup-test variants and the full configured CTest suite pass. Interactive dialog confirmation remains.
